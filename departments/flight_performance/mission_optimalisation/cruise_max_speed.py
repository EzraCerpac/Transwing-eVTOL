...
class CruiseOptMaxSpeed(Optimalisation):
    ...
    def constraints(self):
        ...
        self.opti.subject_to([
            ...
            self.dyn.speed[0] == 45,
            self.dyn.speed[:self.n_timesteps//2] >= 45,#self.dyn.speed[0],
            self.dyn.speed[self.n_timesteps//2:] >= 45,#self.dyn.speed[-1],
            self.dyn.speed[-1] == 45,
            # self.dyn.speed[-1] >= 40,
            self.dyn.gamma[0] == 0,
            self.dyn.gamma[-1] == 0,
    ...
    def cruise_constraints(self):
        ...
        # cruise_altitude = self.opti.parameter(self.aircraft.cruise_altitude)
        ...
        cruise_speed = self.opti.variable(init_guess=self.aircraft.cruise_velocity, log_transform=True)
        self.opti.subject_to([
...
if __name__ == '__main__':
    ...
    ac = rot_wing
    # ac.data.v_stall = 20.
    mission_profile_optimization = CruiseOptMaxSpeed(ac,
                                             opt_param=OptParam.MIN_TIME,
                                             n_timesteps=80,
                                             max_iter=1000)
