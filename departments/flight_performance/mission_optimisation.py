...
class MissionProfileOptimization(Model):
    ...
    def __init__(self, aircraft: Aircraft, opt_param: OptParam, n_timesteps=500):
    ...
    def horizontal_constraints(self):
        ...
        a_z = np.diff(self.dyn.speed * np.sin(self.dyn.gamma)) / np.diff(self.time)
        ...
        start_cruise_index = np.argmin(np.abs(self.dyn.x_e - start_cruise_distance))
        end_cruise_index = np.argmin(np.abs(self.dyn.x_e - end_cruise_distance))
        cruise_altitude = self.opti.variable(init_guess=self.aircraft.cruise_altitude, log_transform=True,
                                             lower_bound=self.aircraft.cruise_altitude)
        cruise_velocity = self.opti.variable(init_guess=self.aircraft.cruise_velocity, log_transform=True)
        self.opti.subject_to([
    ...
    def vertical_constraints(self):
        self.dyn = asb.DynamicsPointMass2DSpeedGamma(
            ...
            z_e=self.opti.variable(init_guess=np.linspace(0, -100, self.n_timesteps),
                                   upper_bound=0),
            speed=self.opti.variable(init_guess=0, n_vars=self.n_timesteps),
            gamma=self.opti.variable(init_guess=0, n_vars=self.n_timesteps, lower_bound=-np.pi / 2,
                                     upper_bound=np.pi / 2),
            alpha=self.opti.variable(init_guess=0, n_vars=self.n_timesteps, lower_bound=-30, upper_bound=30),
        )
        v_x = np.diff(self.dyn.x_e) / np.diff(self.time)
        v_z = self.opti.derivative_of(self.dyn.z_e, with_respect_to=self.time, derivative_init_guess=0)
        # a_x = self.opti.derivative_of(v_x, with_respect_to=self.time, derivative_init_guess=0)
        a_z = self.opti.derivative_of(v_z, with_respect_to=self.time, derivative_init_guess=0)
        alpha_derivative = np.diff(self.dyn.alpha) / np.diff(self.time)
        self.thrust_level = self.opti.variable(init_guess=0, n_vars=self.n_timesteps, lower_bound=0, upper_bound=1)
...
if __name__ == '__main__':
    ...
    ac = Aircraft.load()
    mission_profile_optimization = MissionProfileOptimization(ac, opt_param=OptParam.TIME, n_timesteps=200)
