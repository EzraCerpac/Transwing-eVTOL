...
class MissionProfileOptimization(Model):
    ...
    def init(self, dynamic_func: callable):
        ...
        self.dyn = asb.DynamicsPointMass2DSpeedGamma(
            ...
            z_e=self.opti.variable(init_guess=np.linspace(0, -self.aircraft.cruise_altitude, self.n_timesteps), upper_bound=0),
            speed=self.opti.variable(init_guess=self.aircraft.cruise_velocity, n_vars=self.n_timesteps, log_transform=True),
