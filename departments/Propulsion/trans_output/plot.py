...
@show
@save
def plot_transition_free_variables(time: np.ndarray, velocities: np.ndarray, delta_T: np.ndarray, trans_vals: np.ndarray) -> (plt.Figure, plt.Axes):
    ...
    ax1.text(time[-1] / 10, y, "Vertical configuration", color='tab:green', verticalalignment='top')
    ax1.hlines(0, time[-1] * 9 / 10, time[-1], color='tab:green', linestyle='--')
    ax1.text(time[-1] * 9 / 10, 0, "Horizontal configuration", color='tab:green', verticalalignment='bottom', horizontalalignment='right')
