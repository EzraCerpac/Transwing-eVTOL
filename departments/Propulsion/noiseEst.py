...
# A-weighing
frequencies = np.array([6.3, 8, 10, 12.5, 16, 20, 25, 31.5, 40, 50, 63, 80, 100, 125, 160,
                        200, 250, 315, 400, 500, 630, 800, 1000, 1250, 1600, 2000, 2500,
                        3150, 4000, 5000, 6300])
a_weighting = np.array([-85.4, -77.8, -70.4, -63.4, -56.7, -50.5, -44.7, -39.4, -34.6, -30.2,
                        -26.2, -22.5, -19.1, -16.1, -13.4, -10.9, -8.6, -6.6, -4.8, -3.2,
                        -1.9, -0.8, 0, 0.6, 1, 1.2, 1.3, 1.2, 1, 0.5, -0.1])
...
def harmonicNoise(choice, B):
    ...
    if choice=="sei":
        ...
        config = eightengs()
        configuration = eight
    overallNoise = (configuration["L1"]+20*np.log10(4/B)+40*np.log10(15.5/toimp_dist(config.D))+
           configuration["B3"]+configuration["B8"]-20*np.log10((np.sqrt(r**2+1))-1))
