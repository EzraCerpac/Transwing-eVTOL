...
thrust = 2e4
# parametric.wings[1].set_control_surface_deflections({'Elevator': 5.})
...
def equations_of_motion(t: float, y: np.ndarray) -> tuple[float | ndarray]:
    ...
    thrust_per_engine = thrust / len(parametric.propulsors)
    for propulsor in parametric.propulsors:
        dyn.add_force(
...
dyn.draw(parametric, scale_vehicle_model=5)
