...
def test_convert_array_happy_path():
    ...
    np.testing.assert_array_equal(unit_conversion.convert_array(array, 'meter', 'kilometer'), expected)

def test_convert_array_same_unit():
    array = np.array([1, 2, 3])
    expected = np.array([1, 2, 3])
    np.testing.assert_array_equal(unit_conversion.convert_array(array, 'meter', 'meter'), expected)
...
def test_convert_array_with_pandas_series():
    ...
    pd.testing.assert_series_equal(unit_conversion.convert_array(series, 'meter', 'kilometer'), expected)