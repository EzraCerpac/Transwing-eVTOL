...
def generate_model(trans_val: float | np.ndarray) -> Airplane | np.ndarray:
    """
    ...
    if isinstance(trans_val, np.ndarray):
        assert 0 <= np.any(trans_val) <= 1, "All transition values must be between 0 and 1"
    ...
    for v in trans_val:
        ...
        alpha: float = 45
        beta: float = 40
        q = v * (q_range[1] - q_range[0]) + q_range[0]
    ...
    if len(airplanes) == 1:
        return airplanes[0]
    return np.array(airplanes)


trans_wing = AC(
    name=ac.full_name,
    data=ac,
    parametric_fn=generate_model,
)
