...
class TransitionAnim:
    ...
    def __init__(self, alpha:float=45, beta:float=35) -> None:
        ...
        self.r_prop = [np.array([[0.1],[-0.5],[0]]), np.array([[0.1],[0.1],[0]])]
        # Thrust vector in local coordinates
        self.v_prop = [np.array([[0.2],[0],[0]]), np.array([[0.2],[0],[0]])]

    def set_C_axis(self, q) -> None:

        q = np.deg2rad(q)
        self.C_axis = np.cos(q) * np.eye(3, 3) + np.sin(q) * np.array([[0, -self.axis[2, 0], self.axis[1, 0]], [self.axis[2, 0], 0, -self.axis[0, 0]],[-self.axis[1, 0], self.axis[0, 0], 0]]) + (1 - np.cos(q)) * self.axis * self.axis.T
    ...
    def init_animation(self,) -> None:
        ...
        self.line1 = self.animation_ax.plot([self.A[0], self.B[0]], [self.A[1], self.B[1]], zs=[self.A[2], self.B[2]], c=color)
        self.line2 = self.animation_ax.plot([self.B[0], self.C[0]], [self.B[1], self.C[1]], zs=[self.B[2], self.C[2]], c=color)
        self.line3 = self.animation_ax.plot([self.C[0], self.D[0]], [self.C[1], self.D[1]], zs=[self.C[2], self.D[2]], c=color)
        self.line4 = self.animation_ax.plot([self.A[0], self.D[0]], [self.A[1], self.D[1]], zs=[self.A[2], self.D[2]], c=color)

        self.vectors = []

        for r, v in zip(self.r_prop, self.v_prop):
            tmp_v = r+v
            self.vectors.append(self.animation_ax.quiver(r[0, 0], r[1, 0], r[2, 0], v[0, 0], v[1, 0], v[2, 0]))
    ...
    def draw_wing(self, i) -> None:
        ...
        eff = np.array(eff)
        eff = int(np.sum(eff)/np.sum(np.linalg.norm(self.v_prop, axis=1))*100) #TODO: Test this
