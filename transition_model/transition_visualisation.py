...
def solve():
    ...
    C_axis = cos(q)*np.eye(3,3) + sin(q)*np.array([[0, -z_axis[2,0], z_axis[1,0]], [z_axis[2,0], 0, -z_axis[0,0]], [-z_axis[1,0], z_axis[0,0], 0]]) + (1-cos(q))*z_axis*z_axis.T
    print(C_axis.shape)
    #C_axis = cos(q)*DM.eye(3) + sin(q)*skew #+ (1-cos(q))*z_axis@z_axis.T

    C_axis1 = C_axis@np.array([[-1], [0], [0.0]]) - np.array([[0], [-1], [0.0]])
    C_axis2 = C_axis@np.array([[-1], [0], [0.1]]) - np.array([[-0.1], [-1], [0.0]])

    g = Function('g', [alpha, beta], [C_axis1[0, 0], C_axis1[1, 0], C_axis1[2, 0]])
    ...
    G = rootfinder('G','newton', g, {'print_iteration':False, 'line_search':False, 'abstol':0.0001})
    print(G(0.5, 0.5))
