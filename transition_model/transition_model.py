...
class TransitionModel:
    ...
    def BI(self):
        self.C_BI = self.C_x(self.q[0, 0])@self.C_y(self.q[1, 0])@self.C_z(self.q[2, 0])

    def W1B(self):
        self.C_W1B = self.C_z(self.alpha) @ self.C_x(np.pi/2 - self.beta) @ self.C_z(self.q[6, 0])

    def W2B(self):
        self.C_W2B = self.C_z(self.alpha+np.pi/2) @ self.C_x(np.pi/2 - self.beta) @ self.C_z(self.q[7, 0])
    ...
    def draw(self):
        ...
        print((self.C_W1B))
        axis = Function('axis', [self.q], [(self.C_W1B@np.array([[0],[0],[1]]))[0, 0], (self.C_W1B@np.array([[0],[0],[1]]))[1, 0], (self.C_W1B@np.array([[0],[0],[1]]))[2, 0]])
        print(axis(0))
    ...
    def set_C(self, q: np.ndarray | MX) -> list[np.ndarray | MX]:
        ...
        self.C = []
        C_BA = cos(q) * np.eye(3, 3) + sin(q) * skew(self.A_axis_of_rotation) + (
