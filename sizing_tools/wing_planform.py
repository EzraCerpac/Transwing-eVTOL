...
class wing_model(Model):
    ...
    def rootcrt(self) -> float:
        cr = 2*self.aircraft.wing.area*(1+self.aircraft.taper)/self.aircraft.wing.span   
        return cr
    ...
    def MAC(self)-> float:
        MAC_length = self.rootcrt()*2/3*(1+self.aircraft.taper+self.aircraft.taper**2)/(1+self.aircraft.taper)
        return MAC_length
    
    def Reynolds(self)->float:
        Re = self.rho*self.aircraft.cruise_velocity*self.MAC()/self.mu                          #reynolds engine
        return Re


aircraft = Aircraft()  
model = wing_model(concept_C2_1)
print(model.MAC())
