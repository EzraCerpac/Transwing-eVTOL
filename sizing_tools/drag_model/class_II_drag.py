...
class C2Drag():
    ...
    def param_check(self):
        ...
        # print(R_N_wing)
        if 6E6 > R_N_wing < 5E6:
            raise ValueError("Wing Reynolds number has changed to much, revise the Roskam plots")
        
        R_N_fus = self.Reynolds(self.l_fus)
        # print(R_N_fus)
        if 30E6> R_N_fus < 28E6:
            raise ValueError("Fuselage Reynolds number has changed to much, revise the Roskam plots")

    def C_f_RAYMER(self, l, lam_percent):
        C_f_lam = 1.328 / np.sqrt(self.Reynolds(l))
        C_f_tur = 0.455 / ((np.log10(self.Reynolds(l)))**2.58 * (1 + 0.144*self.mach**2)**0.65)
        return C_f_lam * (lam_percent/100) + C_f_tur * (1 - (lam_percent/100))
    ...
    def total_CD0_RAYMER(self):
        ...
        S_wet_fus = (np.pi * self.d_fus / 4) * (
            (1/(3*L_1**2)) * ((4*L_1**2 + self.d_fus**2/4)**1.5 - self.d_fus**3/8) 
        ...
        FF_wing = (1 + (0.6/self.x_tc*self.MAC)*self.tc + 100 * (self.tc**4))*(1.34*self.mach**0.18*np.cos(self.sweep)**0.28)
        ...
        total_drag = (wing_CD0 + fuselage_CD0 + tail_CD0) * 1.20 # For Excrescence drag and leakage
    ...
    def wing_CD0(self):
        ...
        return R_wf * R_ls * C_f_w * (1+ L_tc + 100*(self.tc)**4) * S_wet_w / self.S
    ...
    def fus_CD0(self):
        ...
        return R_wf * C_f_fus * (1 + 60 /(self.l_fus/self.d_fus)**3 + 0.0025*(self.l_fus/self.d_fus)) * S_wet_fus / self.S 
