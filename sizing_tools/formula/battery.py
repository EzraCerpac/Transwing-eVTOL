...
def mass_from_energy(energy: float, battery_energy_density: float, battery_system_efficiency: float, SoC_min: float) -> float:
    """
    ...
    """
    return energy * (1 + SoC_min) / (convert_float(battery_energy_density, 'kWh', 'W*s') * battery_system_efficiency)