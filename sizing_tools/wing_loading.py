...
class WingLoading:
    ...
    def S_w(self):
        return 2*self.aircraft.total_mass*sc.g/(self.rho*self.aircraft.cruise_velocity^2*self.aircraft.mission_profile[Phase.CRUISE].C_L)
    ...
    def ver_climb(self, T_A, Sref_Sw):
        T_W = 1.2 * (
            1 + 1 /
            (np.arange(1, 2000)) * self.rho * self.aircraft.mission_profile.phases[Phase.CLIMB].vertical_speed**2 * Sref_Sw)
        P_W = (T_W * (1 / (self.aircraft.figure_of_merit * self.aircraft.propulsion_efficiency)) *
               np.sqrt(T_A / (2 * self.rho)))**(-1)
        return P_W

    def steady_climb(self):
        c_l_opt = C_L_climb_opt(self.aircraft.estimated_CD0, self.aircraft.wing.aspect_ratio,self.aircraft.wing.oswald_efficiency_factor)
        c_d = C_D_from_CL(c_l_opt, self.aircraft.estimated_CD0, self.aircraft.wing.aspect_ratio, self.aircraft.wing.oswald_efficiency_factor)
        cd_cl_three_over_2 = c_d/c_l_opt**(3/2)
        W_P = self.aircraft.propulsion_efficiency * (self.aircraft.rate_of_climb + cd_cl_three_over_2 *
    ...
    def plot_wp_ws(self, concept) -> tuple[plt.Figure, plt.Axes]:
        ...
        plt.plot(xx, self._wp(xx, (concept.wing.span**2/concept.wing.area)), label='Cruise')
        plt.plot(xx, self.ver_climb(concept.TA, concept.sref/concept.wing.area), label='Vertical Climb')
        plt.plot(xx,
                 self.steady_climb(),
                 label='Cruise Climb')
        plt.xlabel('W/S')
