...
class EnergySystemMassModel(MassModel):
    ...
    def estimate_energy(self) -> float:
        return sum(self._power(phase) * phase.duration for phase in self.mission_profile.phases)

    def total_mass(self, initial_total_mass: float = None) -> float:
        return self.estimate_energy() * (1 + self.aircraft.SoC_min) / (
                    self.aircraft.battery_energy_density * self.aircraft.battery_system_efficiency)
    ...
    def _power(self, phase: MissionPhase) -> float:
        ...
        P_hv = rotor_disk_thrust ** (3/2) / (self.aircraft.figure_of_merit * sqrt(2 * rho * rotor_disk_area))
        match phase.phase:
            case Phase.TAKEOFF:
                return P_hv
            case Phase.CLIMB:
                ...
                assert phase.vertical_speed / self.aircraft.mission_profile.phases[0].vertical_speed > 0
