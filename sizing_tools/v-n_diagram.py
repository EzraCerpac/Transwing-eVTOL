...
class VNDiagram(Model):
    ...
    def gust_load_slope_Vc(self):
        wing_loading = self.aircraft.total_mass * g / self.aircraft.wing.area
        wing_loading_psf = wing_loading * 0.020885
        Ug = 2 * wing_loading_psf / (
            Atmosphere(self.aircraft.cruise_altitude).density() * 0.00194 *
            self.aircraft.wing.mean_aerodynamic_chord * 3.2808399 * g *
            3.2808399 * CLALPHA)  #TODO CLalpha
        Kg = (0.88 * Ug) / (5.3 + Ug)
        Ude = 50
        slope = (Kg * Ude * CLALPHA) / (498 * wing_loading_psf
                                        )  # slope in [-]/[feet/second]
        return slope
    ...
    def maneuver_diagram(self):
        ...
        list_with_all_loadfactors = np.hstack(
            [N_top, N_bottom, N_top_vc, N_top_vd, N_bottom_vc, N_bottom_vd])

        min_loadfactor = min(np.min(list_with_all_loadfactors),
                             self.N_lim_neg())
        ...
        plt.plot([self.maneuvering_speed(),
                  self.dive_speed()],
                 [self.N_lim_pos(), self.N_lim_pos()],
                 color='black',
                 label='Max loadfactor: ' + str(round(max_loadfactor, 2)))
        plt.plot([self.dive_speed(), self.dive_speed()], [self.N_lim_pos(), 0],
                 color='black')
        plt.plot([self.design_cruise_speed(),
                  self.dive_speed()], [self.N_lim_neg(), 0],
                 color='black',
                 label='Min loadfactor: ' + str(round(min_loadfactor, 2)))
        ...
        plt.plot(V_vd,
                 ...
                 color='black',
                 label='Gust')
        plt.plot([V_vd[-1], V_vc[-1]], [N_top_vd[-1], N_top_vc[-1]],
                 linestyle='dashed',
                 color='black')
        plt.plot([V_vd[-1], V_vc[-1]], [N_bottom_vd[-1], N_bottom_vc[-1]],
                 linestyle='dashed',
                 color='black')
        plt.plot([V_vd[-1], V_vd[-1]], [N_top_vd[-1], N_bottom_vd[-1]],
                 linestyle='dashed',
                 color='black')
