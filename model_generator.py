...
def generate_models(q:np.ndarray, alpha:float, beta:float) -> dict:
    ...
    C_axis = (np.cos(dq) * np.eye(3, 3) + np.sin(dq) * np.array([
        [0, -rot_axis[2, 0], rot_axis[1, 0]],
        [rot_axis[2, 0], 0, -rot_axis[0, 0]],
        [-rot_axis[1, 0], rot_axis[0, 0], 0]
    ]) + (1 - np.cos(dq)) * rot_axis * rot_axis.T)
    
    p_tip_le = np.array([ac.wing.span / 2 * tan(wing_model.le_sweep),
                        ac.wing.span / 2,
                    ac.wing.span / 2 * tand(wing_model.dihedral)])
    p_tip_te = np.array([ac.wing.span / 2 * tan(wing_model.le_sweep)-wing_model.tipcrt,
                ac.wing.span / 2,
                ac.wing.span / 2 * tand(wing_model.dihedral)])

    p_cut_le0 = np.array([ac.wing.span*cut / 2 * tan(wing_model.le_sweep),
                         ac.wing.span*cut / 2,
                         ac.wing.span*cut / 2 * tand(wing_model.dihedral)])
    p_cut_le = np.array([ac.wing.span*cut / 2 * tan(wing_model.le_sweep),
                         ac.wing.span*cut / 2,
                         ac.wing.span*cut / 2 * tand(wing_model.dihedral)])
    
    print(p_cut_le)
    p_cut_te = np.array([ac.wing.span*cut / 2 * tan(wing_model.le_sweep)-wing_model.tipcrt,
                         ac.wing.span*cut / 2,
                         ac.wing.span*cut / 2 * tand(wing_model.dihedral)])
    
    #Prop system location
    p_prop=np.array([ac.wing.span*(0+0.5) / 2 * tan(wing_model.le_sweep), ((0 + .5) / ac.motor_prop_count - .5) * ac.wing.span, 0])
    ...
    for i in q:
        ...
        twist_cut = float(np.rad2deg(np.arccos(v_cut.T@v_cut_0/(np.linalg.norm(v_cut)))))
        print(twist_cut)
